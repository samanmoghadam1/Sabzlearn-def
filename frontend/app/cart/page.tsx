...
const Cart = () => {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      setAlert({ error: "", success: "دوره ها با موفقیت خریداری شدند" });
      window.location.href = "/my-account/courses";
    } catch (error: any) {
      console.log("error: ", error);
      setAlert({ success: "", error: "خطایی رخ داده است" });
    } finally {
      setIsLoading(false);
    }
  }
        ...
        >
          ...
          <div className="mt-2 p-3 rounded-bottom-4">
            ...
            <button
              ...
              style={{ backgroundColor: "var(--sabzlearn-color)" }}
            >
              {isLoading ? "در حال بررسی" : "تکمیل خرید"}
            </button>
          </div>
        </div>
      </div>
